...
def Nepresunute_load():
    soubor = os.path.join(desktop_path, "Nepresunute.dsk")

    try:
        with open(soubor, "r") as f:
            obsah = f.readlines()
            return [tuple(cesta.strip('()\n').split(',')) for cesta in obsah]
    except FileNotFoundError:
        return []
    
def Nepresunute_save(seznam_dvojic):
    soubor = os.path.join(desktop_path, "Nepresunute.dsk")

    with open(soubor, "w") as f:
        for dvojice in seznam_dvojic:
            f.write(f"({dvojice[0]},{dvojice[1]})\n")
...
def Icon_swap(seznam_ploch, vybrana_plocha_path):
    ...
    for soubor in os.listdir(desktop_path):
        if soubor not in nemenitelnost:
            try:
                ...
                nepresunuto.append((puvodni_cesta, cilova_cesta))
    ...
    Nepresunute_save(nepresunuto)

    print("Ikony byly přesunuty mezi plochami. Poslední použitá plocha: {}".format(z_plocha))
    pass  

seznam_ploch = Seznam_ploch_load()
nepresunuto = Nepresunute_load()
nemenitelnost = seznam_ploch + ["Desktoper.py", "desktop_index.dsk", "Plochy.dsk", "Nepresunute.dsk"]
...
def Dopresun(seznam_nepresunutejch):
    if seznam_nepresunutejch != []:
        for puvodni_cesta, cilova_cesta in seznam_nepresunutejch:
            try:
                shutil.move(puvodni_cesta, cilova_cesta)
                nepresunuto.remove((puvodni_cesta, cilova_cesta))
            except:
                print("Nepovedlo se přesunout: " + puvodni_cesta)
